...
export default function ArticleCard({ backgroundImage, date, title, href }) {
  ...
  const bottomDivClass = `absolute bottom-0 left-0 w-full h-1/4 bg-black bg-opacity-80 ${transitionClass} ${
  ...
  const renderHoverContent = () => (
    <div className="text-white p-4 flex flex-col justify-between h-3/4 pt-2">
      <div className="overflow-hidden line-clamp-5 text-gray-300 text-sm">
        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Veniam asperiores iste totam vero earum aliquid ducimus vel
        dolor delectus distinctio et, facere quos illum temporibus recusandae illo pariatur, ad labore.
      </div>
